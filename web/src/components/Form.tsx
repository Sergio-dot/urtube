  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `/api/v1/search/${encodeURIComponent(query)}?limit=${limit.value}`,
      );
      if (!response.ok) {
  ...
  return (
    <Listbox value={selected} onChange={onChange}>
      <div className="relative">
        <ListboxButton className="grid w-full cursor-default grid-cols-1 rounded-2xl bg-white/5 py-4 pr-2 pl-4 text-left text-white ring-1 ring-inset ring-white/10 focus:ring-2 focus:ring-inset focus:ring-indigo-500 sm:text-sm/6 transition-all">
        ...
        <ListboxOptions
          transition
          className="absolute z-10 mt-2 max-h-56 w-full overflow-auto rounded-xl bg-gray-900 py-1 text-base ring-1 ring-white/10 data-leave:transition data-leave:duration-100 data-leave:ease-in data-closed:data-leave:opacity-0 sm:text-sm"
        >
          {options.map((option) => (
            <ListboxOption
              key={option.id}
